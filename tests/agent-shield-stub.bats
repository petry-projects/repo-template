...
#     standard trigger set — it is required so this stub's `agent-shield /
#     AgentShield` check reports on a merge queue's `gh-readonly-queue/*` ref
#     (see standards/ci-standards.md). Do not drop it on sync and do not flag it
#     as drift.
